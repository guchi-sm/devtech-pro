...
const PROJECTS = [
  { id: 1, index: '001', category: 'Software Dev', title: 'Inventory Management System',
    ...
    tags: ['PHP', 'MySQL', 'Bootstrap', 'JavaScript', 'Chart.js'], year: '2024', duration: '8 weeks', outcome: 'Reduced stockout incidents by 60%' },
  { id: 2, index: '002', category: 'Web Dev', title: 'Business Website',
    ...
    tags: ['HTML5', 'CSS3', 'JavaScript', 'Figma', 'SEO'], year: '2024', duration: '3 weeks', outcome: '40% increase in online inquiries' },
  { id: 3, index: '003', category: 'Networking', title: 'Small Office Network Setup',
    ...
    tags: ['Cisco', 'MikroTik', 'Cat6 Cabling', 'VLAN', 'Ubiquiti'], year: '2023', duration: '2 weeks', outcome: '99.8% uptime in first 6 months' },
  { id: 4, index: '004', category: 'IT Support', title: 'IT Infrastructure Overhaul',
    ...
    tags: ['Windows Server', 'Active Directory', 'Cloud Backup', 'IT Policy'], year: '2023', duration: '6 weeks', outcome: 'Zero data loss incidents post-migration' },
  { id: 5, index: '005', category: 'Software Dev', title: 'HR Management Portal',
    ...
    tags: ['React', 'Node.js', 'PostgreSQL', 'Tailwind CSS'], year: '2024', duration: '10 weeks', outcome: 'Cut HR admin time by 35%' },
  { id: 6, index: '006', category: 'Networking', title: 'Retail Chain WiFi Deployment',
    ...
    tags: ['Ubiquiti UniFi', 'VLAN', 'Captive Portal', 'Site Survey'], year: '2023', duration: '4 weeks', outcome: 'Full coverage across all 4 sites' },
]
  ...
  return (
    <Reveal delay={delay}>
      <div className="group cursor-pointer overflow-hidden"
        style={{ background: '#fff', border: `1px solid ${hovered ? '#f5a623' : 'var(--border)'}`, transition: 'border-color .3s, box-shadow .3s, transform .3s', transform: hovered ? 'translateY(-4px)' : 'translateY(0)', boxShadow: hovered ? '0 16px 40px rgba(28,45,63,.12)' : 'none' }}
        onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
        <div className="relative h-52 overflow-hidden">
          <img src={proj.img} alt={proj.title} loading="lazy" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent" />
          <div className="absolute top-3 right-3 font-mono text-[0.58rem] tracking-[0.15em] uppercase px-2.5 py-1" style={{ background: '#f5a623', color: '#1c2d3f', fontWeight: 700 }}>{proj.category}</div>
          <div className="absolute bottom-4 left-5 font-display text-5xl leading-none" style={{ color: 'rgba(255,255,255,.15)' }}>{proj.index}</div>
          <div className="absolute bottom-0 left-0 right-0 p-4 transition-transform duration-300"
            style={{ background: '#1c2d3f', transform: hovered ? 'translateY(0)' : 'translateY(100%)', borderTop: '3px solid #f5a623' }}>
            <p style={{ color: '#f5a623', fontSize: '.75rem', fontWeight: 700 }}>✓ {proj.outcome}</p>
          </div>
        </div>
        <div style={{ position: 'relative', overflow: 'hidden' }}>
          <img src={proj.img} alt="" aria-hidden="true" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: hovered ? 0.07 : 0.04, filter: 'blur(2px) saturate(0.5)', transition: 'opacity .4s', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', inset: 0, background: hovered ? 'linear-gradient(135deg, rgba(28,45,63,.06) 0%, rgba(245,166,35,.04) 100%)' : 'rgba(255,255,255,.92)', transition: 'background .4s', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: hovered ? 3 : 0, background: '#f5a623', transition: 'width .3s' }} />
          <div className="p-6" style={{ position: 'relative', zIndex: 1 }}>
            <div className="font-mono text-[0.6rem] tracking-[0.2em] uppercase mb-2" style={{ color: '#f5a623', fontWeight: 700 }}>{proj.year} · {proj.duration}</div>
            <h3 className="font-display text-xl tracking-wide mb-2.5" style={{ color: '#1c2d3f' }}>{proj.title}</h3>
            <p className="text-xs leading-relaxed mb-4" style={{ color: 'var(--text-muted)' }}>{proj.desc}</p>
            <div className="flex flex-wrap gap-1.5 mb-4">{proj.tags.map(tag => <span key={tag} className="tech-tag">{tag}</span>)}</div>
            <a href="#" onClick={e => e.preventDefault()} className="inline-flex items-center gap-2 font-mono text-[0.65rem] tracking-[0.12em] uppercase transition-colors duration-200"
    ...
    <>
        ...
        <div className="max-w-[1280px] mx-auto px-8 md:px-10">
          <div className="flex flex-col gap-3 py-5">
            <div className="relative max-w-sm">
              ...
              <input type="text" placeholder="Search projects, tags, tech..." value={query}
            ...
            <div className="flex flex-wrap gap-1">
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => setActiveFilter(cat)} className="font-mono text-[0.65rem] tracking-[0.14em] uppercase px-4 py-2 transition-all duration-200"
                  style={{ background: activeFilter === cat ? '#f5a623' : 'transparent', color: activeFilter === cat ? '#1c2d3f' : 'var(--text-muted)', border: `1px solid ${activeFilter === cat ? '#f5a623' : 'var(--border)'}`, fontWeight: activeFilter === cat ? 700 : 400, borderRadius: 2 }}>
                  {cat}
                </button>
              ))}
            </div>
      ...
      <section className="py-10 pb-24" style={{ background: '#f4f6f9' }}>
        <div className="max-w-[1280px] mx-auto px-8 md:px-10">
          ...
          <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {filtered.map((proj, i) => <ProjectCard key={proj.id} proj={proj} delay={i * 0.08} />)}
          </motion.div>
        </div>
      </section>
      ...
      <section style={{ position: 'relative', padding: '5rem 0', overflow: 'hidden' }}>
        ...
        <div className="max-w-[1280px] mx-auto px-8 md:px-10 flex flex-col md:flex-row items-center justify-between gap-6" style={{ position: 'relative', zIndex: 1 }}>
          <Reveal>
            <div>
              <div style={{ fontSize: '1.6rem', fontWeight: 800, color: '#fff', marginBottom: '.5rem', lineHeight: 1.2 }}>Have a project in mind?</div>
              <p style={{ fontSize: '.95rem', color: 'rgba(255,255,255,.65)' }}>Let's discuss how I can help bring your idea to life.</p>
            </div>
          </Reveal>
          <Reveal delay={0.15}>
